...
-(CGFloat)getLableHeightWith:(id<TrySecondModelProtocol>)model{
    CGSize size = [[model showText] boundingRectWithSize:CGSizeMake(150, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading | NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]}  context:nil].size;
...
-(void)changeStateWithSource:(id<TrySecondModelProtocol>)model completeHandl:(void (^)(id<TrySecondModelProtocol>, BOOL))block{
    ...
    TypeTwoModel *twoModel = model;
    twoModel.twoIsTap = !twoModel.twoIsTap;
    block(twoModel,YES);
}
@end
