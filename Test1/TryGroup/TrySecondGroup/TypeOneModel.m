...
-(CGFloat)getLableHeightWith:(id<TrySecondModelProtocol>)model{
    CGSize size = [[model showText] boundingRectWithSize:CGSizeMake(220, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading | NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:20]}  context:nil].size;
    return size.height>=20?size.height:20;
}

-(CGFloat)getRowHeightWith:(id<TrySecondModelProtocol>)model{
    
    return 10+[self getLableHeightWith:model]+10;
}
...
-(void)changeStateWithSource:(id<TrySecondModelProtocol>)model completeHandl:(void (^)(id<TrySecondModelProtocol>, BOOL))block{
    ...
    TypeOneModel *oneModel = model;
    oneModel.oneIsTap = !oneModel.oneIsTap;
    block(oneModel,YES);
}
